 ...
 */
if ( ! class_exists( 'Themebase_Customize' ) ) {
	class Themebase_Customize {
		...
		public function change_inline_stylesheet() {
			return 'themebase-style';
		}
		/**
		 * Add customize preview css
		 */
		public function customize_preview_css() {
			wp_enqueue_style( 'kirki-custom-css', THEMEBASE_THEME_URI . '/assets/admin/css/customizer.min.css' );
		}
		/**
		 * Load Customizer.
		 */
		public function load_customizer() {
			Themebase::require_file( THEMEBASE_THEME_DIR . '/customizer/customizer.php' );
			...
			if ( ! is_customize_preview() && $count === 2 ) {
				...
				if ( ! empty( $presets ) ) {
					foreach ( $presets as $preset ) {
						$page_preset_value = Themebase_Helper::get_post_meta( $preset, '-1' );
